...
echo "[Entrypoint] MySQL Docker Image mysql-community-minimal-release-1.2.14-server"
...
EOF
	...
	if [ -n "$MYSQL_INITIALIZE_ONLY" ]; then
		...
		echo "[Entrypoint] Starting MySQL mysql-community-minimal-release-1.2.14-server"
	fi
